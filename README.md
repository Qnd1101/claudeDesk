...
- 👁️ **삭제 전 미리보기(FR-08):** `p`로 우측 패널에 세션 대화 스니펫을 띄워 **`claude`를 켜지 않고** 내용 확인 후 지울지 결정. 파일 전체 로드 없이 스트리밍(RAM 바운드).
...
| `S` | 정렬 방향 토글 (↓/↑) |
| `g` | 평면/프로젝트 그룹 뷰 토글 (`Tab`/`Enter` 그룹 접기·펼치기) |
| `p` | 미리보기 패널 토글 (선택 세션 대화 스니펫, 터미널 ≥100칸) |
| `Space` | 다중 선택 토글 (✓) · 그룹 헤더에선 그룹 일괄 선택 |
