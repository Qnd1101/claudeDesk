...
| **v2.2.0** | **2026-06-25** | **포지셔닝 재정의:** `claude --resume` 내장 피커와 차별점이 약한 "이어하기 보조" 프레이밍을 폐기하고, **세션 하우스키핑(띄우지 않고 정리·삭제·그룹핑)을 헤드라인 가치로 승격**. Elevator Pitch·핵심 가치(§2) 재배치, RAM/검색/정렬은 보조 속성으로 강등. FR-09(그룹핑) M2 내 우선 구현으로 당김. |
...
10. **용어·FR ID 정합.** 본 문서의 FR ID(FR-01~FR-14)를 Task/UIUX 문서와 1:1 일치시킴.

---

## 2. 프로젝트 개요 및 배경 (Executive Summary & Background)

* **제품 요약 (Elevator Pitch):** Claude Code 세션을 **`claude`를 실행하지 않고** 관리·정리·삭제하는 standalone **세션 하우스키핑** TUI 툴. 안전 삭제·휴지통 복구·프로젝트 그룹핑·메타 점검을 전담하고, 이어하기(resume)는 보조 기능으로 제공.
* **포지셔닝 / 차별점 (Positioning):**
  * `claude --resume`의 **내장 세션 피커**는 "이어할 세션을 고르는 것"까지만 한다 — 검색·정렬·미리보기가 거기에 일부 흡수되므로 그것만으로는 **차별점이 약하다**.
  * claudeDesk의 **해자(moat)는 반대편**: 세션을 **띄우지 않고 청소하는 일**(안전 삭제·휴지통·복구·프로젝트 단위 그룹핑·메타 점검). 내장 피커가 다루지 않는 영역이며 `claude`를 켜지 않고 수행한다.
  * 따라서 헤드라인 가치는 **하우스키핑**이고, RAM·검색·정렬은 그 일을 **빠르고 안전하게** 만드는 보조 속성으로 재배치한다.
* **개발 배경 (Problem Statement):**
  * Claude Code 세션이 `~/.claude/projects/`에 수백 개 누적되지만, 기본 CLI로는 **쌓인 세션을 띄우지 않고 정리·삭제·그룹핑할 수단이 없다** — 식별·재진입은 내장 피커가 일부 해결하나 "청소"는 사각지대.
  * 기존 AI 관리 GUI는 Electron 기반이 많아 RAM 소모가 큼. **Docker·IDE·빌드툴을 동시 구동하는 리소스 타이트 환경**에서도 가볍게 도는 전용 관리자가 필요.
* **핵심 가치 (Core Values):**
  1. **Session Housekeeping** — `claude`를 띄우지 않고 세션을 정리·삭제·그룹핑. 내장 피커가 다루지 않는 핵심 영역. (FR-04/09/11)
  2. **Non-Destructive** — 원본 JSONL은 **읽기 전용·이동만**. 삭제는 소프트(휴지통)·2단계 영구삭제. 부가 정보는 사이드카 분리. (부록 D, CI SHA 불변 강제)
  3. **Privacy First** — 외부 서버 전송 0. 100% 로컬. 텔레메트리 없음.
  4. **Ultra-Low RAM** — 메모리 풋프린트 최소화 (Target RSS < 20MB, §5.1에서 측정 정의) — 하우스키핑을 가볍게 만드는 보조 속성.
...
| **FR-11** | 휴지통/복구 | 소프트 삭제된 세션 목록 조회·복구·영구삭제(**사람 명시 확인 전용**). 보관기간(메타 기록)은 표시용이며 **자동 영구삭제는 하지 않음**(안전핀 §9.3). | Should | M2 | FR-04와 한 쌍 |
...
| **FR-14** | 오래된 세션 정리 | 기준일(7/30/90/180/365일) 이전 수정된 비활성 세션을 `o`로 일괄 **선택**(대상 수 미리표시). 삭제는 FR-04 흐름에 위임 — **선택 ≠ 삭제, 자동 삭제 없음**(안전핀 §9.3). | Could | M3 | FR-04·FR-07과 연동 |
...
| 휴지통 보관기간 경과 | 삭제시각 메타만 기록·표시. **자동 영구삭제 없음**(안전핀 §9.3 — 사람 명시 확인 전용). 자동 만료 정리는 향후(M3) 옵트인 검토 |
...
5. **휴지통 보관기간 기본값.** 30일은 **메타 표시용**으로만 기록(설정화). 경과분 **자동 영구삭제는 하지 않음**(안전핀 §9.3 — 사람 확인 전용). 자동 만료 정리는 M3 옵트인 검토. **(해소)**
